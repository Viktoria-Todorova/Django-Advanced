...
class NoteDetailsView(UserPassesTestMixin,LoginRequiredMixin,DetailView):
    queryset = Note.objects.select_related('owner')

    def test_func(self):
        return self.request.user.has_perm('notes.can_access_all_notes') or self.get_object().owner == self.request.user
